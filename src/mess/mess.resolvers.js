...
import {
    generateMongoFilterFromMessItemFilter,
    generateMongoSortFromMessItemOrderBy,
} from "./mess.utils.js";
    ...
    Query: {
        messItems: async (_parent, args) => {
            ...
            const mongoFilter = filter
                ? generateMongoFilterFromMessItemFilter(filter)
                : {};
            const mongoSort = orderBy
                ? generateMongoSortFromMessItemOrderBy(orderBy)
                : {};
    ...
    Mutation: {
        ...
        deleteMessItem: async (_parent, args) => {
            ...
            const mongoFilter = filter
                ? generateMongoFilterFromMessItemFilter(filter)
                : {};
            const { n, ok, deletedCount } = await MessItem.deleteMany(
                mongoFilter
            );
